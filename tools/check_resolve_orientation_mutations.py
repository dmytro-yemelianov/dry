#!/usr/bin/env python3
"""Compile bounded resolve-orientation source mutations and require the named proof fixture to kill each one."""
...
MANIFEST = ROOT / "proofs" / "resolve-orientation-mutations-v0.toml"
FIXTURE = ROOT / "proofs" / "fixtures" / "resolve-orientation-refinement-v0.json"
EXPECTED_MODEL = "resolve-orientation-source-mutations-v0"
TEST_NAME = "native_resolve_orientation_refines_generated_lean_corpus"
...
def run_test(workspace: Path, witness: str) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        "test",
        "--test",
        "resolve_orientation_refinement",
        "--",
        TEST_NAME,
    ]
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="dry-resolve-orientation-mutation-") as temp_dir:
        ...
        print(f"error: cannot run resolve orientation source mutations: {error}", file=sys.stderr)
        return 1

    print(
        f"resolve orientation source mutations: ok "
