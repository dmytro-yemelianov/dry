...
- **Report goldens** (`conformance/reports/`): `UPDATE_REPORTS=1 cargo test -p kmet-trace --test report_goldens`
...
- **KRL program golden** (`conformance/reports/robot/`): `UPDATE_GOLDEN=1 cargo test -p dry-core --test krl_program_structure`.
  ...
  (see `docs/22-krl-emit.md`). CI runs it too (the `krl` job), so a regenerated golden that is no
  longer KRL fails there; running it locally first needs a JRE and `antlr4-python3-runtime`.
