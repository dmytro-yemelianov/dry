...
crates/
  core/          the dependency-free Dry IR + engine (no PyO3/numpy)   [P0.1, stub present]
  cli/           the `dry` command (inspect/verify/emit/optimise)      [P1.6]
  py/            the PyO3 binding                                       [P1.6]
  wasm/          the wasm-bindgen binding                              [P3.4]
sdk/
  python/        the Python authoring SDK                              [P2.4]
  ts/            the TypeScript authoring SDK                          [P4.1]
conformance/     corpora ported from the FullControl fork + the runner [P0.4/P0.5]
```
