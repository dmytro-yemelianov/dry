...
use dry_core::clothoid::{corner_blend, fresnel, fresnel_with_terms, ClothoidError, CornerBlend};
use dry_core::{
...
fn a_blend_that_exactly_fills_its_leg_is_not_pose_stable() {
    ...
    let placed = |degrees: f64| {
        ...
        let program: dry_core::FeatureProgram = serde_json::from_str(&text).unwrap();
        let expanded = dry_core::expand_features(&program).expect("feature expands");
