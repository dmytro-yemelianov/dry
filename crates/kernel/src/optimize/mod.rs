...
/// way (see `apply_gated`, which supplies the verification policy from outside this crate): a span's
/// rewrite is kept only when it introduces no new error rule (`docs/11-profiles-and-reports.md`).
...
/// `apply_gated` / `apply_safe_gated`, which live above the kernel because they need the verifier).
#[derive(Debug, Clone)]
...
/// verifier. `apply_gated`, one layer up, supplies the verification policy that makes this the L2
/// rewrite gate.
