...
mod tests {
    ...
    /// The `max-flow` half of this claim — that the same prime move trips the flow rule — needs
    /// layer 2 and runs as `e_only_prime_moves_are_flagged_over_the_flow_limit` in
    /// `crates/core/tests/gcode_lift_verified.rs` (plan Task 4).
    #[test]
