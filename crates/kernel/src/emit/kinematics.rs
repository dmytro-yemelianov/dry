...
///
/// Public only because [`KinematicsExt::resolve_joints`] returns it across the crate boundary; the
/// inner pair stays private, so a caller can carry a resolution from `resolve_joints` to
/// `rotary_words` and no more (plan Task 4).
#[derive(Debug, Clone, Copy)]
pub struct Joints([f64; 2]);
