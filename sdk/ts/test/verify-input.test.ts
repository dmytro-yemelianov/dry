...
test('verify accepts structured bounds/speedRange and agrees with the CSV form', () => {
  ...
  const structured = d.verify({ bounds: [[0, 100], [0, 100], [0, 50]], speedRange: [300, 9000] });
  const csv = d.verify({ bounds: '0,100,0,100,0,50', speedRange: '300,9000' });
...
test('verify surfaces retraction and first-layer findings from structured limits', () => {
  ...
  const report = d.verify({
    maxRetractionDistance: 2, // mm
    maxRetractionSpeed: 1000, // mm/min
    maxTravelWithoutRetract: 30, // mm
    firstLayerHeightRange: [0.3, 0.5], // mm
    firstLayerSpeedRange: [2000, 3000], // mm/min
  });
...
test('verify first-layer ranges accept legacy CSV strings equal to the structured form', () => {
  ...
  const structured = d.verify({ firstLayerHeightRange: [0.3, 0.5], firstLayerSpeedRange: [2000, 3000] });
  const csv = d.verify({ firstLayerHeightRange: '0.3,0.5', firstLayerSpeedRange: '2000,3000' });
