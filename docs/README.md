...
| [`21-mathematical-assurance-plan.md`](21-mathematical-assurance-plan.md) | Proof-oriented implementation plan for Dry language semantics: exact theorem boundaries, floating-point error budgets, Rust refinement, compiler-pass contracts, codecs, verification, targets and honest assurance claims. |
| [`22-krl-emit.md`](22-krl-emit.md) | The KUKA (KRL) target: the BNF Dry emits, the external grammar it is checked against (and the survey of what KRL validators exist at all), the orientation/velocity/`CIRC` conventions with sources — and the boundary that none of it has ever run on a controller. |
| [`adr/`](adr/) | Architecture decision records, beginning with the formal-assurance constitution and claim vocabulary. |
