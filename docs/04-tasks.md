...
- `[~]` **FM1.3** Transform, feature and frame proofs. **Landed for the bounded planar subset:** generic `Feature`/ordered `Group`/`Repeat` expansion semantics; source-order and repeat decomposition laws; exact expanded-operation counts; operation/node/depth budget preservation; deterministic traces; exact-real pose action on abstract points, vectors and invariant operations; checked success/first-error semantics for tool, partial-coordinate move, arc, spline, orientation and manual-code operations; 17 schema-valid Lean-generated fixtures consumed independently by Rust; 11 declared semantic negative controls distinguished by the corpus; six registered feature claims, with ordering, repeat, budgets and success/error determinism marked refinement-checked only for the committed corpus. **Remaining:** non-finite pose/name/operation and counter-overflow behavior, a broader source-level mutation sweep, checked binary64 bounds, then `SE(3)` poses and named frame graphs after D1.3. *Dep: FM1.2, P2.3; full scope D1.3. Accept:* transform action, repeat, ordering, budget, determinism and frame-resolution/rejection theorems pass with Rust refinement fixtures.
