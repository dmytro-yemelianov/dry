...
- `[~]` **P3.1** (L) Optimisation passes on L2 — semantics-preserving IR→IR transforms with invariant tests. **Landed:** `merge_collinear` (`crates/core/src/optimize.rs`) — coalesces consecutive collinear same-state moves; `simulate` metrics (time / volume / distance / peak flow) are preserved, only the segment count drops; idempotent; CLI `dry optimize`. *Deferred:* `arc_fit` (fit G2/G3 to polylines within tolerance), `travel_reorder`, `adaptive_speed`, `coasting`, `z_hop`. *Accept (for landed):* each pass conserves its invariants; material Δ ≤ 1e-12.
