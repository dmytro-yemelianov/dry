...
- `[ ]` **P0.2** (M) Define **Dry IR v0** Rust types: L2 motion dialect (toolframe = position+orientation, channels), typed quantities (`Length`/`Speed`/`Volume`/`Flow`/`Temperature`/`Angle`). *Dep: P0.1. Accept:* types compile; unit arithmetic mismatches are compile errors; round-trips through serde.
- `[ ]` **P0.3** (M) JSON + binary (columnar) encodings of Dry IR v0 with a versioned header (units/provenance/invariants). *Dep: P0.2. Accept:* `from(to(ir)) == ir`; binary ≥3× smaller than JSON on a large fixture (the fork measured 3.84×).
