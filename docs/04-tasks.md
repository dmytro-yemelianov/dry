...
- `[~]` **FM1.3** Transform, feature and frame proofs. **Landed for the bounded abstract planar subset:** generic `Feature`/ordered `Group`/`Repeat` expansion semantics; source-order and repeat decomposition laws; exact expanded-operation counts; operation/node/depth budget preservation; deterministic traces; exact-real pose action on abstract points, vectors and invariant operations; executable ordering/repeat/budget snapshots; five registered claims with explicit numeric/refinement boundaries. **Remaining:** concrete operation validation and error traces, generated Rust differential/mutation cases, checked binary64 bounds, then `SE(3)` poses and named frame graphs after D1.3. *Dep: FM1.2, P2.3; full scope D1.3. Accept:* transform action, repeat, ordering, budget, determinism and frame-resolution/rejection theorems pass with Rust refinement fixtures.
