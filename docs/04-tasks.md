...
- `[~]` **FM1.1** Assurance constitution and Lean tooling spike. **Landed locally:** accepted ADR 0001; schema-backed claim registry and independent validator; pinned Lean 4/Mathlib project; checked exact-real planar transform composition and ordered-fold determinism proofs; dedicated CI job. **Remaining:** observe the new job green from a clean GitHub runner and record an independent clean-checkout reproduction before closing the milestone. *Dep: P4.3 (met). Accept:* planar transform composition and deterministic fold proofs build in CI; every registered claim names assumptions, relation, semantic version and implementation link.
- `[~]` **FM1.2** Quantity, syntax and well-formedness kernel. **Landed:** exact dimension exponent algebra, dimension-indexed quantities, same-dimension units with positive rational scales, canonical conversion/normalization coherence, and a checked deposition-dimension identity. **Remaining:** versioned logical L1/L2 syntax, structured failures, decidable well-formedness and future public quantity vocabulary after D1.2. *Dep: FM1.1; future quantities also depend on D1.2. Accept:* normalization/coherence/dimension theorems and validity decision equivalence are checked.
