# Dry Cloud — Project Handover (ON HOLD as of 2026-07-28)

Owner paused the project after Task R3. This document is the resume point. Read it with:
...
## Next task at resume: R4 (usage metering + quotas)

Plan section `### Task R4`. Key reconciliation the implementer must do: the jobs route
already enforces `QUOTA_JOBS_PER_MONTH` — pick ONE canonical quota source (jobs table
vs `usage_events`), align the 429 shape (`Retry-After` = seconds to next UTC month) and
add `GET /v1/usage`. Then R5 (CLI `dry auth` + `dry cloud verify` — mock-server tests,
mirror `crates/cli/src/printer_registry.rs` idioms), R6 (docs), R7 (deploy — see
below), R8 (v0.5.0 release).
