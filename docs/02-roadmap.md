...
| **`v0.11.0`** | **Post-audit remediation** | Engine-delegated compatibility across bindings, fail-closed cloud ingress, executable standalone/release test gates, capability-manifest completeness, honest proof-strength metadata, and direction-sensitive RAPID `MoveC` geometry with structural drift coverage. | **Target** |
